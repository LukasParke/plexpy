...
class CreateTransientTokenGlobalsTypedDict(TypedDict):
...
class CreateTransientTokenGlobals(BaseModel):
    accepts: Annotated[
...
class CreateTransientTokenRequestTypedDict(TypedDict):
    media_type: QueryParamType
    r"""The value `delegation` is the only supported `type` parameter."""
    scope: Scope
    r"""The value `all` is the only supported `scope` parameter."""
    accepts: NotRequired[components_accepts.Accepts]
...
class CreateTransientTokenRequest(BaseModel):
...
class CreateTransientTokenMediaContainerTypedDict(TypedDict):
...
class CreateTransientTokenMediaContainer(BaseModel):
    r"""`MediaContainer` is the root element of most Plex API responses. It serves as a generic container for various types of content (Metadata, Hubs, Directories, etc.) and includes pagination information (offset, size, totalSize) when applicable.
    Common attributes: - identifier: Unique identifier for this container - size: Number of items in this response page - totalSize: Total number of items available (for pagination) - offset: Starting index of this page (for pagination)
    The container often \"hoists\" common attributes from its children. For example, if all tracks in a container share the same album title, the `parentTitle` attribute may appear on the MediaContainer rather than being repeated on each track.
    """
    ...
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["identifier", "offset", "size", "totalSize", "token"])
        serialized = handler(self)
        m = {}
...
class CreateTransientTokenResponseBodyTypedDict(TypedDict):
    r"""OK"""

    media_container: NotRequired[CreateTransientTokenMediaContainerTypedDict]


class CreateTransientTokenResponseBody(BaseModel):
    r"""OK"""

    media_container: Annotated[
        Optional[CreateTransientTokenMediaContainer],
...
class CreateTransientTokenResponseTypedDict(TypedDict):
    content_type: str
    ...
    object: NotRequired[CreateTransientTokenResponseBodyTypedDict]
    r"""OK"""


class CreateTransientTokenResponse(BaseModel):
    content_type: str
    r"""HTTP response content type for this operation"""
    ...
    object: Optional[CreateTransientTokenResponseBody] = None
    r"""OK"""
...
try:
    CreateTransientTokenMediaContainer.model_rebuild()
except NameError:
    pass
try:
    CreateTransientTokenResponseBody.model_rebuild()
except NameError:
    pass
