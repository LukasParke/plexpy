...
class ResetPlayQueueGlobalsTypedDict(TypedDict):
...
class ResetPlayQueueGlobals(BaseModel):
    accepts: Annotated[
...
class ResetPlayQueueRequest(BaseModel):
...
class ResetPlayQueueResponseTypedDict(TypedDict):
    content_type: str
