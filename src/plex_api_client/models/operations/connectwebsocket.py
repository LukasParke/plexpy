...
class ConnectWebSocketGlobalsTypedDict(TypedDict):
...
class ConnectWebSocketGlobals(BaseModel):
    accepts: Annotated[
...
class ConnectWebSocketRequestTypedDict(TypedDict):
    accepts: NotRequired[components_accepts.Accepts]
    r"""Indicates the client accepts the indicated media types"""
    client_identifier: NotRequired[str]
    r"""An opaque identifier unique to the client"""
    ...
    r"""By default, all events except logs are sent. A rich filtering mechanism is provided to allow clients to opt into or out of each event type using the `filters` parameter. For example:
...
class ConnectWebSocketRequest(BaseModel):
    accepts: Annotated[
        Optional[components_accepts.Accepts],
        FieldMetadata(header=HeaderMetadata(style="simple", explode=False)),
    ] = components_accepts.Accepts.APPLICATION_XML
...
class ConnectWebSocketResponseTypedDict(TypedDict):
    content_type: str
...
class ConnectWebSocketResponse(BaseModel):
    content_type: str
    r"""HTTP response content type for this operation"""
