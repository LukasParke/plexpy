...
class AddToPlayQueueGlobalsTypedDict(TypedDict):
...
class AddToPlayQueueGlobals(BaseModel):
    accepts: Annotated[
...
class AddToPlayQueueRequestTypedDict(TypedDict):
    play_queue_id: int
    ...
    r"""The marketplace on which the client application is distributed"""
    uri: NotRequired[str]
    r"""The content URI for what we're adding to the queue."""
    playlist_id: NotRequired[str]
    r"""The ID of the playlist to add to the playQueue."""
    next: NotRequired[components_boolint.BoolInt]
    r"""Play this item next (defaults to 0 - queueing at the end of manually queued items)."""


class AddToPlayQueueRequest(BaseModel):
...
class AddToPlayQueueResponseTypedDict(TypedDict):
    content_type: str
