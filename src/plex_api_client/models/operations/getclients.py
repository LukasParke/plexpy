...
class GetClientsGlobalsTypedDict(TypedDict):
...
class GetClientsGlobals(BaseModel):
    accepts: Annotated[
...
class GetClientsRequestTypedDict(TypedDict):
    accepts: NotRequired[components_accepts.Accepts]
    r"""Indicates the client accepts the indicated media types"""
    client_identifier: NotRequired[str]
    r"""An opaque identifier unique to the client"""
...
class GetClientsResponseBodyTypedDict(TypedDict):
    ...
    server: NotRequired[List[components_plexdevice.PlexDeviceTypedDict]]


class GetClientsResponseBody(BaseModel):
    r"""`MediaContainer` is the root element of most Plex API responses. It serves as a generic container for various types of content (Metadata, Hubs, Directories, etc.) and includes pagination information (offset, size, totalSize) when applicable.
    Common attributes: - identifier: Unique identifier for this container - size: Number of items in this response page - totalSize: Total number of items available (for pagination) - offset: Starting index of this page (for pagination)
    The container often \"hoists\" common attributes from its children. For example, if all tracks in a container share the same album title, the `parentTitle` attribute may appear on the MediaContainer rather than being repeated on each track.
    """
    ...
    server: Annotated[
        Optional[List[components_plexdevice.PlexDevice]], pydantic.Field(alias="Server")
    ] = None

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["identifier", "offset", "size", "totalSize", "Server"])
        serialized = handler(self)
        m = {}
...
class GetClientsResponseTypedDict(TypedDict):
    content_type: str
    ...
    object: NotRequired[GetClientsResponseBodyTypedDict]
    r"""List of clients"""
...
class GetClientsResponse(BaseModel):
    ...
    object: Optional[GetClientsResponseBody] = None
    r"""List of clients"""
...
try:
    GetClientsResponseBody.model_rebuild()
except NameError:
    pass
