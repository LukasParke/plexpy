...
class DVRResponseMediaContainerMediaContainerTypedDict(TypedDict):
...
class DVRResponseMediaContainerMediaContainer(BaseModel):
    r"""`MediaContainer` is the root element of most Plex API responses. It serves as a generic container for various types of content (Metadata, Hubs, Directories, etc.) and includes pagination information (offset, size, totalSize) when applicable.
    Common attributes: - identifier: Unique identifier for this container - size: Number of items in this response page - totalSize: Total number of items available (for pagination) - offset: Starting index of this page (for pagination)
    The container often \"hoists\" common attributes from its children. For example, if all tracks in a container share the same album title, the `parentTitle` attribute may appear on the MediaContainer rather than being repeated on each track.
    """
...
class DVRResponseMediaContainerTypedDict(TypedDict):
    media_container: NotRequired[DVRResponseMediaContainerMediaContainerTypedDict]
    dvr: NotRequired[List[DvrTypedDict]]


class DVRResponseMediaContainer(BaseModel):
    media_container: Annotated[
        Optional[DVRResponseMediaContainerMediaContainer],
...
class DVRResponseTypedDict(TypedDict):
    media_container: NotRequired[DVRResponseMediaContainerTypedDict]


class DVRResponse(BaseModel):
    media_container: Annotated[
        Optional[DVRResponseMediaContainer], pydantic.Field(alias="MediaContainer")
    ] = None
...
try:
    DVRResponseMediaContainerMediaContainer.model_rebuild()
except NameError:
    pass
try:
    DVRResponseMediaContainer.model_rebuild()
except NameError:
    pass
try:
    DVRResponse.model_rebuild()
except NameError:
    pass
