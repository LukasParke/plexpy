...
class Timeline(BaseSDK):
    ...
    def get_conversion_queue(
        ...
        *,
        request: Union[
            operations.GetConversionQueueRequest,
            operations.GetConversionQueueRequestTypedDict,
        ],
        retries: OptionalNullable[utils.RetryConfig] = UNSET,
        server_url: Optional[str] = None,
        timeout_ms: Optional[int] = None,
        http_headers: Optional[Mapping[str, str]] = None,
    ) -> operations.GetConversionQueueResponse:
        r"""Get Conversion Queue
        ...
        if not isinstance(request, BaseModel):
            request = utils.unmarshal(request, operations.GetConversionQueueRequest)
        request = cast(operations.GetConversionQueueRequest, request)
        ...
        req = self._build_request(
            ...
            accept_header_value="application/json",
            http_headers=http_headers,
            _globals=operations.GetConversionQueueGlobals(
                accepts=self.sdk_configuration.globals.accepts,
                client_identifier=self.sdk_configuration.globals.client_identifier,
                product=self.sdk_configuration.globals.product,
                version=self.sdk_configuration.globals.version,
        ...
        http_res = self.do_request(
            hook_ctx=HookContext(
                config=self.sdk_configuration,
                base_url=base_url or "",
                operation_id="getConversionQueue",
                oauth2_scopes=None,
                security_source=self.sdk_configuration.security,
                tags=["Timeline"],
                extensions=None,
        ...
        if utils.match_response(http_res, "200", "application/json"):
            return operations.GetConversionQueueResponse(
                media_container_with_play_queue=unmarshal_json_response(
                    Optional[components.MediaContainerWithPlayQueue], http_res
                ),
                status_code=http_res.status_code,
                content_type=http_res.headers.get("Content-Type") or "",
                raw_response=http_res,
    ...
    async def get_conversion_queue_async(
        self,
        *,
        request: Union[
            operations.GetConversionQueueRequest,
